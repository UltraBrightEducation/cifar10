...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image-size", type=int, default=224, help="Output size of the image"
    )
    ...
    with tf.python_io.TFRecordWriter(args.output_path, options=options) as writer:
        ...
        for image_id, label in tqdm(image_dict.items(), total=len(image_dict)):
            image_paths = [
                join(args.image_path, get_channel_filename(image_id, channel))
                for channel in CHANNELS
            ]
            image = stack_func(image_paths)

            feature = {
                "label": _int64array_feature(label),
